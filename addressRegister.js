class AddressRegister extends Module {
  constructor(bus, address, load, reset, clock, programSignal, x, y) {
    ...
    this.programAddressBits = createArrayOfSignals(bus.length/2);
    ...
    super.addGate(new Selector(this.programAddressBits, this.outputs, programSignal, address));
    for(let i = 0; i < this.programAddressBits.length; i++) {
      let led = new Led(i*25 + x, y + 25, 10, this.programAddressBits[i], (Math.pow(2,(this.programAddressBits.length - i - 1))).toString(), BOTTOM);
      super.addRender(led);
      super.addClick(led);
    }
